...
func windowDragHandleShouldCaptureHit(
    ...
    if let window = dragHandleView.window,
       WindowDragHandleHitTestState.isResolvingTopHit(window: window) {
        windowDragHandleEmitBreadcrumb(
            "titlebar.dragHandle.hitTest.reentrant",
            window: window,
            eventType: eventType,
            point: point,
            minInterval: 10,
            extraData: [
                "hit_resolution_depth": WindowDragHandleHitTestState.depth(window: window)
            ]
        )
    }
       ...
       !WindowDragHandleHitTestState.isResolvingTopHit(window: window) {
        ...
        if let topHit {
            ...
            if topHitBelongsToTitlebarOverlay && !isPassiveHostHit {
                windowDragHandleEmitBreadcrumb(
                    "titlebar.dragHandle.hitTest.blockedByTopHit",
                    window: window,
                    eventType: eventType,
                    point: point,
                    minInterval: 8,
                    extraData: [
                        "top_hit_type": String(describing: type(of: topHit)),
                        "top_hit_in_overlay": true,
                        "top_hit_passive_host": false
                    ]
                )
                return false
            }
